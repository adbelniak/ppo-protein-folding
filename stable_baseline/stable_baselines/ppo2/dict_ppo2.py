...
from stable_baselines.common.runners import AbstractDictEnvRunner
...
class DictRunner(AbstractDictEnvRunner):
    def __init__(self, *, env, model, n_steps, gamma, lam):
        ...
        self.lam = lam
        self.gamma = gamma
        self.best_distance = tf.placeholder(tf.float32, [None], name="best_distance")
        self.writer = None
        self._write_best_distance = self.get_best_distance_writer()

    def get_best_distance_writer(self, obs_path="protein_to_observe.txt"):
        protein_to_observe = None
        with open(obs_path) as file:
            protein_to_observe = file.read().split(',')

        average_by_name = {}
        for protein_name in protein_to_observe:
            average_by_name[protein_name] = []

        def _write_best_distance(dones, infos, num_timesteps):
            average_by_name = {}
            for protein_name in protein_to_observe:
                average_by_name[protein_name] = []
    ...
    def _run(self):
        ...
        for _ in range(self.n_steps):
            ...
            self._write_best_distance(self.dones, infos, self.model.num_timesteps // self.n_envs)
